...
async function main() {
  ...
  const brainBox = new THREE.Box3()
  brainBox.setFromObject(brain)

  const brainBoxHelper = new THREE.Box3Helper(brainBox, new THREE.Color('white'))
  interactionManager.add(brain)
  scene.add(brainBoxHelper)

  brain.addEventListener('click', () => {
    console.log('hovering bounding box')
  })
...
async function makeBrain() {
  ...
  const gltf = await gltfLoader.loadAsync('/models/brain_sliced_old.glb')
